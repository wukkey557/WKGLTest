	...
	) 
{
	...
	while (true) 
	{
		...
		*/
		...
		rst.drawTriangle(v, image1);
		rst.drawTriangle(v1, image1);*/
		/*
			纹理end	 ===========<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
		*/
		...
		Vert vertArr[] = 
		{
			{ 10, 10, 0, 2.0f, Rgba(0, 0, 0) },
			{ 210, 210, 2.0f, 0, Rgba(0, 0, 0) },
			{ 10, 210, 0, 0,  Rgba(0, 0, 0) },

			{ 210, 10, 2.0f, 2.0f, Rgba(0, 0, 0) },
			{ 210, 210, 2.0f, 0, Rgba(0, 0, 0) },
			{ 10, 10, 0, 2.0f, Rgba(0, 0, 0) }
		};
		...
		step += 1.0f;
		matrix3 mat;
		mat.translate(step, step);
		for ( int i = 0; i < 6; i++ )
		{
			float3 p = float3( vertArr[i].x, vertArr[i].y, 1 );
			p = mat * p;
			vertArr[i].x = p.x;
			vertArr[i].y = p.y;
		}*/
		...
		mat.rotate(step);
		matrix3 mat1;
		mat1.translate(step, step);
		
		// matrix3 matFinal = mat * mat1;
		matrix3 matFinal = mat;
		rst.loadMatrix(matFinal);
		// rst.loadIdentify();
		image1->setWrapType(1);
		rst.vertexPointer(2, DT_FLOAT, sizeof(Vert), &vertArr[0]);
		rst.textureCoordPointer(2, DT_FLOAT, sizeof(Vert), &vertArr[0].u);
		rst.colorPointer(4, DT_BYTE, sizeof(Vert), &vertArr[0].color);
		rst.bindTexture(image1);
		rst.drawArrays(Raster::DM_TRIANGLE, 0, 6);
