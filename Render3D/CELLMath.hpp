...
namespace WKGL
{
    ...
    */
    ...
    public:
		...
		Rgba4Byte& operator + ( const Rgba4Byte& right ) 
		{
			if ( right._r > 255 - this->_r )
				this->_r = 255;
			else 
				this->_r += right._r;

			if (right._g > 255 - this->_g)
				this->_g = 255;
			else
				this->_g += right._g;

			if (right._b > 255 - this->_b)
				this->_b = 255;
			else
				this->_b += right._b;

			if (right._a > 255 - this->_a)
				this->_a = 255;
			else
				this->_a += right._a;
	...
	inline tvec2<float> uvLerp(const tvec2<float>& uv1, const tvec2<float>& uv2, float s)
	{
		tvec2<float> p;
		p.x = uv1.x + s * (uv2.x - uv1.x);
		p.y = uv1.y + s * (uv2.y - uv1.y);
		return p;
	}
